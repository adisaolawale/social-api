...
const ErrorCodes = require('../constants/errorCodes');   // ← Add this

function errorHandler(err, req, res, next) {
    let statusCode = err.statusCode || 500;
    let message = err.message || 'Something went wrong';
    let errorCode = err.errorCode || null;        // ← NEW
    let details = err.details || null;            // ← NEW

    // ... (keep all your existing PostgreSQL, JWT, Multer error handling)

    // Example: Add errorCode for common cases
    if (err.code === '23505') {  // unique violation
        statusCode = 409;
        message = 'A record with that value already exists';
        errorCode = ErrorCodes.ACCOUNT_ALREADY_EXISTS;
    }
    ...
    return errorResponse(res, { 
        message, 
        statusCode,
        errorCode,      // ← Send to frontend
        details         // ← Extra data if needed
    });
}
