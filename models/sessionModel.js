...
const SessionModel = {
  ...
  // logut device
  // ============================
  invalidateSessionByHashToken: async (hashedToken) => {
    const query = `
      UPDATE sessions
      SET is_valid = FALSE
      WHERE token_hash = $1
    ...
    await dbQuery(query, [hashedToken]);
  },
...
rotateSession: async ({ oldRefreshToken, newRefreshToken, expiresAt }) => {
    ...
    try {
        ...
        // 1. Find and update existing session (DO NOT create new one)
